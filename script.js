...
async function planRoute() {
    ...
    try {
        ...
        if (rData.code === 'Ok') {
            L.geoJSON(rData.routes[0].geometry, { style: { color: '#3498db', weight: 6, opacity: 0.8 } }).addTo(map);
            timeInfo.innerHTML = "⏱️ Fahrzeit: " + Math.floor(rData.routes[0].duration / 60) + " Min.<br>📏 Strecke: " + (rData.routes[0].distance / 1000).toFixed(1) + " km";
